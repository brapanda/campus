<template>
    ...
    <view class="ju-field-item">
        ...
        <view class="ju-field-container"
              :class="{'has-border':border||mode==='default','is-cube':mode==='cube','is-focus':isFocus,'is-error':instanceErrorText,'is-disabled':disabled}" :style="{backgroundColor}">
            <!--下划线-->
            ...
            <label class="ju-field-label" @click="isFocus=true" :class="labelMoreClass" :style="{minWidth:labelWidth,color:instanceErrorText?'#ff6155':(isFocus?focusBorder:'')}" v-if="label">
        ...
        computed: {
            labelMoreClass: function () {
                ...
                if (this.labelBgIsColor) {
                    _cla += ` bg-${this.labelBackgroundColor.replace(/(^\s*)|(\s*$)/g, "")}`
                }
                return _cla
            },
