<template>
	<view class="chose-city">
		<!-- 城市搜索 -->
		<view class="city-search-wrap" style="padding-top: 100rpx;">
			<view class="search">
				<view class="l-search">
					<view class="icon-search">
						<view class="cuIcon-search"></view>
					</view>
					<input class="input-search" type="text" :value="inputValue" placeholder="请输入学校" placeholder-style="color:#8E8F97"
			...
			<view class="reach-content" v-show="inputValue">
				<block v-show="searchData.length">
					<view class="li" v-for="item in searchData" :key="item.citycode" @click="selectCity(item)">
						<view style="line-height: 70rpx;">{{ item.name }}</view>
						<view style="line-height: 0rpx;">{{ item.nameCn}}</view>
					</view>
				</block>
		 ...
		 @scroll="scrollHandle" v-if="!inputValue">
			<view class="block">
				...
				<view class="area list-item" id="record" v-if="recordList.length">
					<view class="title-wrapp">
						<view class="c-title">
							<text class="l">热门学校</text>
						</view>
					</view>
					<view class="ul">
						<view class="li font-clamp" style="font-size: 25upx;font-weight: 600;" v-for="item in recordList" :key="item.citycode" @click="selectCity(item)">
							{{ item.nameCn }}
						</view>
					</view>
				</view>
			...
			<view class="city-list">
				<view class="list list-item" v-for="(item, key) of cityList" :key="key" :id="item.nameType">
					...
					<view class="item" v-for="innerItem in item.list" :key="innerItem.citycode" @click="selectCity(innerItem)">
						<view style="line-height: 70rpx;">{{ innerItem.name }}</view>
						<view style="line-height: 0rpx;">{{ innerItem.nameCn}}</view>
					</view>
				</view>
			</view>
		</scroll-view>
					...
					this.cityList.map(v => {
						v.list.forEach((item) => {
							if (/^[a-zA-Z]+$/.test(item.pinyin) && item.pinyin.toLowerCase().includes(this.inputValue.toLowerCase()) ||
								item.name.toLowerCase().includes(this.inputValue.toLowerCase()) || item.nameCn.includes(this.inputValue)) {
								result.push(item);
							}
						});
					})
